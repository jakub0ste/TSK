...
def calculate_forecast(p, q, df):
    ...
    # Add labels to display selected p and q values
    p_value_label = ttk.Label(frame, text=f"Selected p: {p}")
    p_value_label.pack(side=tk.LEFT, padx=5)
    q_value_label = ttk.Label(frame, text=f"Selected q: {q}")
    q_value_label.pack(side=tk.LEFT, padx=5)
